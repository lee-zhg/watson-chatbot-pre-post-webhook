...
#### Step 1.1 - Develop Source Code for Translation Logic
...
#### Step 1.2 - Deploy a Service as IBM Cloud Function
...
##### Step 1.2.1 - Deploy Cloud Function
...
1. Create a Cloud Function Namespace. Execute
    ...
    ```
    ibmcloud fn namespace create team1a-ns --description "namespace for team1a"
    ```
...
1. Set the new namespace as default
    ...
    ```
    ibmcloud fn property set --namespace team1a-ns
    ```

1. Create package

    ```
    ibmcloud fn package create multi-language-pkg
    ```

1. Create action for setup using Node.js environment

    ```
    ibmcloud  fn  action  create  multi-language-pkg/preMessageTranslate  preMessageTranslate.js  --kind nodejs:12  --param-file  data/sample_pre_payload.json  --web  true  --web-secure  <YOURSECRET>
    ```
...
##### Step 1.2.2 - Gather Cloud Function Information
...
1. Select the main `Navigation Menu` (often referred as Hamburge menu) at the top-left corner.
    ...
    !["watson-assistant-multi-language-architecture"](docs/images/ibmcloud01.png)
...
1. Copy the CURL command at the bottom. It should look like
    ...
    ```
1. The authentication credential to access the cloud function is embedded as part of the command. In this example, `6e1bf-a76e-4bc2-9c24-8784af9:Vc1jv13mD6nJEIqouEFPwQjLkAnnJ6GzDvTBr3P2` is the username and password. 
...
1. Take note of the credential.


#### Step 1.3 - Configure Pre-Webhook of Watson Assistant Service
...
1. Create an `Assistant` on the `Assistant` tab. Name it `photo-shop-assistant`.
...
1. Enter header information of the pre-webhook.
    ...
    f) Select `Add header` link again.
    
    g) Enter `Content-Type` in the `Header name` field.
    
    h) Enter `Content-Type` in the `Header value` field.

    i) Tab out the field. This saves the pre-webhook configuration changes.
...
1. Create action for a service for post-webhook using Node.js environment
    ...
    ```
    ibmcloud  fn  action  create  multi-language-pkg/postMessageTranslate  postMessageTranslate.js  --kind nodejs:12  --param-file  data/sample_post_payload.json  --web  true  --web-secure  <YOURSECRET>
    ```
...
#### Step 2.3 - Configure Post-Webhook of Watson Assistant Service
...
1. Open `photo-shop-assistant` Assistant.
...
1. Enter header information of the post-webhook.
    ...
    d) Enter user name and password for the post-webhook cloud function. 
    ...
    i) Tab out the field. This saves the post-webhook configuration changes.
...
### Step 4 - Quickly Port Chatbot Developed in one Language to a Different Lanaguage
...
#### Step 4.1 - Duplicate the Skill and Set the Language
...
### Step 4.4 - Quickly Port to Spanish
